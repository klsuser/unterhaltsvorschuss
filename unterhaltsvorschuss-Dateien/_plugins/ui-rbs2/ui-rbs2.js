...
function getRBSConnection(){
    ...
    //return new Promise(function(resolve, reject) {
        ...
        url_ = 'https://fbinter.stadt-berlin.de/rbs/rbs-str-liste-xml.jsp?strname=x&' + (new Date()).getTime();
...
function initRBS(){
    ...
    optionhnr = {
        script: function () {return "https://fbinter.stadt-berlin.de/rbs/rbs-data-xml.jsp?strnr=" + $('#id-input-strnr').val() + '&otnr='+ $('#id-input-otnr').val()},
        varclass: 'house',
        timeout:10000,
        maxentries: 2500,
        minchars: 0,
    ...
    optionstr = {
        script: "https://fbinter.stadt-berlin.de/rbs/rbs-str-liste-xml.jsp?",
        varname: "strname",
        varclass: "street",
        filter: plz.val(),
        timeout:3000,
	...
	if (city.val() != undefined){
		if (city.val().toLowerCase() == 'berlin'){
			...
			if (strnr.val().length > 0){ //&& plz.val().length > 0){ //&& hnrh.val().length > 0){// && beznr.val().length < 1){
				var requestURL = 'https://fbinter.stadt-berlin.de/rbs/rbs-data-xml.jsp?strnr=' + strnr.val() + '&hausnr=' + hnrh.val();
				var request = new XMLHttpRequest();
				request.onreadystatechange = function (){
	...
	if (city.val() != undefined){
		if (city.val().toLowerCase() == 'berlin'){
			...
			if (strnr.val().length > 0 || strs.val().length > 0){ //|| strnr.val.length > 0){ //&& plz.val().length > 0){ //&& hnrh.val().length > 0){// && beznr.val().length < 1){
                var requestURL = "https://fbinter.stadt-berlin.de/rbs/rbs-str-liste-xml.jsp";
